...
// No lid-opening frame sheet exists (only 2 static frames), so each state
// swap is sold with a stylized scale "pop" + tint flash instead of a real
// flipbook — same "bounded lifetime, self-resetting" idiom as `hitFlash` in
// world-guard.tsx / the lerp in axe-projectile.tsx.
const CHEST_POP_DURATION_MS = 350
const CHEST_POP_MAGNITUDE = 0.15
const CHEST_TINT_FLASH_MS = 180
const CHEST_TINT = "#ffe066"

const OPEN_SOUNDS = ["/sounds/chest-open.mp3"]
const CLOSE_SOUNDS = ["/sounds/chest-close.mp3"]
 ...
 *
 * Genuinely re-openable/closable (unlike the earlier one-way design) — once
 * guards are cleared, "E" toggles it back and forth freely with its own
 * open/close sound each way. The relic itself is granted at most once,
 * gated on the inventory store (not on this visual toggle), so re-closing
 * and reopening never re-grants it.
 */
export function WorldChest({ interactPressedRef, onNearChange }: WorldChestProps) {
  ...
  function toggle() {
    ...
    // The relic is granted at most once — gated on the inventory store
    // itself (idempotent), not on this visual open/close toggle, so
    // re-closing and reopening the chest never re-grants it.
